	...
	<div class="container medium-container">
		...
		?>
			<div class="col-xs-12 col-sm-3 col-md-3 searchsection" style="float:right;"><!--  Start Nav tabs -->
				...
				<select id="pagiselector" onchange="changeGrade('<?php echo site_url()."/?page_id=251&searchid="; ?>')">
				...
				<select id="gradeselector" onchange="changeGrade('<?php echo site_url()."/?page_id=251&searchid="; ?>')">
				...
				<ul  class="nav nav-tabs" role="tablist" id="show">
					<h4 class="brands-title">brands</h4>
						<?php  
							...
							<li><img src="<?php echo get_template_directory_uri(); ?>/images/list-icon.png" alt="<?php echo $brand_type->slug;?>"><a href="javascript:void(0)" data-link="<?php echo site_url()."/?page_id=251&branch="; ?>" data-type="<?php echo $brand_type->slug;?>"><span><?php echo $brand_type->slug;?></span></a></li>
						 <?php endforeach; ?>
						  <li><img src="<?php echo get_template_directory_uri(); ?>/images/list-icon.png"><a href="javascript:void(0)" data-link="<?php echo site_url()."/?page_id=251&branch="; ?>" data-type="default"><span>Default</span></a></li>
				</ul>
			</nav>
			</div> <!--  End Nav tabs --> 
			<div class="col-xs-12 col-sm-9 col-md-9" id="specificDiv">
					<?php 	
						...
						$query             =sprintf("
										...
										WHERE vp.post_status='publish' AND vp.post_type='car' AND vtr.`term_taxonomy_id`=13) as first
										inner join
										(SELECT * FROM vc_terms AS vt
										INNER JOIN vc_term_relationships vtr
										ON vt.`term_id`=vtr.`term_taxonomy_id`
										where vt.`name`='%s'
										)as second
										on first.ID=second.object_id) AS brand
										INNER JOIN
										(SELECT vp.* FROM vc_posts AS vp 
										INNER JOIN
										vc_postmeta AS vm
										...
										AND vp.post_type='car' %s
										) AS grade
										ON brand.id=grade.id
										group by brand.id 
						...
						INNER JOIN vc_term_relationships AS vtr
						ON vp.`ID`= vtr.`object_id`
						WHERE vp.post_status='publish' AND vp.post_type='car' AND vtr.`term_taxonomy_id`=13");
							$total_query     = "SELECT COUNT(1) FROM (${query}) AS combined_table";
							$total             = $wpdb->get_var( $total_query );
							$items_per_page = $paginumber;
							$page             = isset( $_GET['cpage'] ) ? abs( (int) $_GET['cpage'] ) : 1;
							...
							$customPagHTML     = "";
								...
								AND vm.`meta_value`='%s' and vtr.term_taxonomy_id=13",$para);
							$total_query     = "SELECT COUNT(1) FROM (${query}) AS combined_table";
							$total             = $wpdb->get_var( $total_query );
							$items_per_page = $paginumber;
							$page             = isset( $_GET['cpage'] ) ? abs( (int) $_GET['cpage'] ) : 1;
